...
(function(document) {
  ...
  window.addEventListener('WebComponentsReady', function() {
    ...
    .then(function(response){ addSearchHeader(response.query); 
                              console.log(response);
                              return getJSON(response.results[0]); 
    })
    .catch(function(){throw Error("Search Request Error"); })
    .then(function(response){ createPlanetThumb(response); })
    .catch(function(error){ addSearchHeader('unknow'); console.log(error) });
  });
})(document);
